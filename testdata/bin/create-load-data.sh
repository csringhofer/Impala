...
if [ $SKIP_METADATA_LOAD -eq 0 ]; then
  # Using Hive in non-parallel mode before before starting parallel execution
  # may help with some flakiness during data load, see IMPALA-8841. The problem
  # only occurs in Hive 3 environment, but always doing the warm up shouldn't
  # hurt much and may make it easier to investigate future issues where Hive doesn't
  # work at all.
  warm-up-hive
