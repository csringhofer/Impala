...
void KrpcDataStreamSender::Channel::TransmitDataCompleteCb() {
  ...
  // this channel can't be in the idle list and won't be used by a concurrent Add() call.
  bool send_eos = false;
  OutboundRowBatch* next_batch =
      queue_->RpcFinished(last_batch, this, status, closed, &send_eos);
  // RpcFinished never sets send_eos on a non-null return
  DCHECK(next_batch == nullptr || !send_eos);
  // Holds a non-OK status if starting the next RPC (TransmitData or EOS) failed.
  // Set inside the lock, used after the lock is released to propagate to the queue.
  Status start_err;
  {
    std::unique_lock<SpinLock> l(lock_);
    callback_uses_queue_ = false;
    if (!status.ok() || ShouldTerminate()) {
      // RPC failed or channel is shutting down; notify Teardown() that the queue is no
      // longer in use.
      rpc_done_cv_.notify_one();
      return;
    }
    if (next_batch != nullptr) {
      ...
      if (UNLIKELY(!next_status.ok())) {
        MarkDone(next_status);
        // Fall through to call queue_->SetError() after releasing the lock.
      } else {
        return;
      }
    } else if (send_eos) {
      // RpcFinished() never sets send_eos=true when the receiver is closed.
      DCHECK(!closed);
      ...
      // No more work for this channel; notify Teardown() if it is waiting for
      // rpc_in_flight_ to clear.
      rpc_done_cv_.notify_one();
      return;
    }
    // Starting the next RPC failed. Capture the status before releasing the lock, then
    // propagate it to the queue outside the lock to respect lock ordering
    // (OutboundQueue::lock_ must not be acquired while Channel::lock_ is held).
    // rpc_done_cv_ was already notified by MarkDone() above, satisfying Teardown().
    start_err = rpc_status_;
  }
  // Propagate the error to the queue outside the channel lock.
  queue_->SetError(start_err);
}
...
Status KrpcDataStreamSender::Channel::SendEosAsync() {
  {
    ...
    // between WaitUntilEmpty() unblocking and this call acquiring lock_.
...
Status KrpcDataStreamSender::WaitForCapacity(
  ...
  // Check queue depth before acquiring batch_pool_lock_ to preserve lock ordering:
  // OutboundQueue::lock_ must not be acquired while holding batch_pool_lock_.
  // If the queue is already at capacity, force reuse of a pooled batch instead of
  // allocating a new one. batch_pool_cv_ is only signalled by ReleaseBatch(), which
  // always adds a batch to free_batch_pool_, so the loop exits via the
  // free_batch_pool_.empty() condition once a buffer becomes available.
  bool must_reuse_batch = (queue != nullptr
      && queue->Size() >= FLAGS_data_stream_sender_per_channel_queue_depth);
...
OutboundRowBatch* KrpcDataStreamSender::OutboundQueue::RpcFinished(
  ...
  *send_eos = false;
  ...
  // channel will never send them. 'batch' may be at any position in the queue when
  // a channel is pipelined to send a non-front batch while the front is still
  // in-flight on other channels. For non-closing channels, also capture the next
  // queued entry: this channel should send it immediately after 'batch'.
  ...
  if (next_queued != nullptr) return next_queued;
