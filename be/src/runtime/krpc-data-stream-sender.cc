...
Status KrpcDataStreamSender::Channel::TransmitData(
  ...
  COUNTER_ADD(parent_->outbound_row_batches_sent_counter_, 1);
  std::unique_lock<SpinLock> l(lock_);
...
Status KrpcDataStreamSender::Channel::FinelizeAndSendBatch(
    std::unique_ptr<OutboundRowBatch>* outbound_batch) {
  ...
  Status ToOutboundRowBatch(OutboundRowBatch* dest) {
    {
      ...
      RETURN_IF_ERROR(dest->PrepareForSend(NUM_TUPLES_PER_ROW,
          compress ? parent_->compression_scratch_.get(): nullptr, false));
...
Status KrpcDataStreamSender::Prepare(
  ...
  outbound_row_batches_sent_counter_ =
      ADD_COUNTER(profile(), "OutboundRowBatchesSent", TUnit::UNIT);
