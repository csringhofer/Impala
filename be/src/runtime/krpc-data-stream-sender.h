...
class KrpcDataStreamSender : public DataSink {
 ...
 private:
  ...
  struct PartitionRowCollector {
    ...
    std::unique_ptr<OutboundRowBatch> in_flight_batch_; // only used in parallel broadcast
    ...
    // TransmiteData(). This call may block if channel_'s preceding RPC is still
