...
bool Tuple::TryDeepCopy(uint8_t** dst_start, const uint8_t* dst_end,
  ...
  dst += desc.byte_size();
  if (!dst_tuple->TryDeepCopyStrings(&dst, dst_end, desc, convert_ptrs)) {
    return false;
  }
  if (!dst_tuple->TryDeepCopyCollections(&dst, dst_end, desc, convert_ptrs)) {
    return false;
  }
  *dst_start = dst;
  return true;
...
bool Tuple::TryDeepCopyStrings(uint8_t** data, const uint8_t* data_end,
  ...
  for (; slot != desc.string_slots().end(); ++slot) {
    ...
    char* new_ptr = convert_ptrs ? nullptr : reinterpret_cast<char*>(*data);
...
bool Tuple::TryDeepCopyCollections(uint8_t** data, const uint8_t* data_end,
    const TupleDescriptor& desc, bool null_ptrs) {
  ...
  for (; slot != desc.collection_slots().end(); ++slot) {
    ...
    coll_value->ptr = null_ptrs ? nullptr : coll_data;
    ...
    *data += coll_byte_size;
    ...
    for (int i = 0; i < coll_value->num_tuples; ++i) {
      ...
      if(!dst_item->TryDeepCopyStrings(data, data_end, item_desc, null_ptrs)) {
        return false;
      }
      if(!dst_item->TryDeepCopyCollections(
          data, data_end, item_desc, null_ptrs)) {
        return false;
      }
