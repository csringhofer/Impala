...
bool Tuple::TryCopyStrings(uint8_t** dst, const uint8_t* dst_end, int* offset,
...
bool Tuple::TryCopyCollections(uint8_t** dst, const uint8_t* dst_end, int* offset,
    const TupleDescriptor& desc, bool convert_ptrs) {
  ...
  for (; slot != desc.collection_slots().end(); ++slot) {
    ...
    if (!TryMemCopy(*dst, dst_end, coll_byte_size, coll_value->ptr)) return false;
    uint8_t* coll_data = reinterpret_cast<uint8_t*>(*dst);
    ...
    *offset += coll_byte_size;
    ...
    for (int i = 0; i < coll_value->num_tuples; ++i) {
      ...
      if(!dst_item->TryCopyStrings(dst, dst_end, offset, item_desc, convert_ptrs)) {
        return false;
      }
      if(!dst_item->TryCopyCollections(dst, dst_end, offset, item_desc, convert_ptrs)) {
        return false;
      }
...
int64_t Tuple::VarlenByteSize(const TupleDescriptor& desc, bool assume_smallify) const {
  int64_t result = StringsByteSize(desc, assume_smallify);
  result += CollectionsByteSize(desc, assume_smallify);
  return result;
}
